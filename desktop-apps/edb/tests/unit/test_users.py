...
def test_create_duplicate_user(engine):
    ...
    with pytest.raises(ValueError, match="already exists"):
        mgr.create_user(UserCreate(username="alice", password="OtherPass456!@#"))
...
def test_ensure_admin_exists(engine):
    mgr = UserManager(engine)
    mgr.ensure_admin_exists("admin", "AdminPass123!@#")
    admin = mgr.get_by_username("admin")
    assert admin is not None
    assert admin.role == Role.ADMIN
    mgr.ensure_admin_exists("admin", "AdminPass123!@#")  # idempotent
    users = mgr.list_users()
